...
data <- read_excel("fisherExact_C2_KEGG.xlsx", 4)
pVAL <- c()
...
pathwayName <- read_excel("results_KEGG_Updated.xlsx", 4)
...
volcano + theme_classic() + theme(legend.position="None") + annotate("text", x = c(1.2, 1.2), y = c(2.7, 5.8), size = 3, label = c("JAK_STAT_SIGNALING", "CYTOKINE_CYTOKINE_RECEPTOR"), color = "blue", fontface = 2)

ggsave("volcano_Pattern_1122_1234_Vs_KEGG_Pathways.pdf")
...
plotDF <- na.omit(plotDF)
volcano = ggplot(data = plotDF, aes(x = fc, y = logP))
volcano + geom_point() + xlab("log2fc") + ylab("-log10(P)") + xlim(-2,2) + ggtitle("module vs random") +
  theme(plot.title = element_text(hjust = 0.5))
...
myDiff1p <- myDiff1p %>%
  mutate(threshold = factor(case_when(fc > 0.58 & logP > 1.5 ~ "cond1",
                                      fc < -0.58 & logP > 1.5 ~ "cond2",
                                      TRUE ~ "cond3")))
...
volcano <-  ggplot(data=myDiff1p, aes(x = fc, y = logP)) + 
  ...
  xlim(c(-2.5, 2.5)) +
  ylim(c(0, 2.9)) +
  scale_color_manual(name="Threshold", values = c("cond1" = "blue", "cond2" = "green", "cond3" = "black"))

# volcano + theme_classic() + theme(legend.position="None") +  annotate("text", x = c(-1,1), y = c(6,6), size = 5, label = c("Lower in Pathway","Higher in Pathway"))

volcano + theme_classic() + theme(legend.position="None") + annotate("text", x = c(1.2, 1), y = c(2.72, 1.9), size = 3, label = c("CYTOKINE_CYTOKINE_RECEPTOR", "JAK_STAT_SIGNALING"), color = "blue", fontface = 2)

ggsave("volcano_Pattern_1122_1234_Vs_KEGG_ModuleSpace.pdf")
